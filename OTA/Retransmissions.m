...
for b = 1:num_tests
    ...
    for a = 1:num_av
        ...
        for t = 1:T
            ...
            eta = eta_h;
            %Calculate error
            est_sum = sum(sqrt(p).*abs(h).*s/sqrt(eta)) + w/sqrt(eta);
            est_sum_h = est_sum_h + est_sum;
            %HENRIK DYNAMIC
        ...
        h_error = real(true_sum-est_sum_h/T)^2;
...
legend("xiaowen", "henrik dynamic", 'Location', 'northwest');
xlabel("\sigma_w")
